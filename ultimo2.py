...
for url in urls:
    ...
    # Busca las solicitudes con 'playlist.m3u8' en la respuesta
    for request in driver.requests:
        if request.response:  # Solo solicitudes con respuesta
            # Verifica que la URL contenga 'playlist.m3u8' y que la respuesta sea JSON
            if "playlist.m3u8" in request.url and request.response.headers.get('Content-Type', '').startswith('application/json'):
                try:
                    ...
                    # En caso de que no se pueda parsear la respuesta como JSON, ignora esta respuesta
                    continue
